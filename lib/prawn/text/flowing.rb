module Prawn
  module Text
    module Flowing
      ...
      # the flow of a document (for captions, labels, charts, etc.), use
      # Text::Box or its convenience method text_box.
      # 
      # Draws text on the page. Prawn attempts to wrap the text to fit within
      # your current bounding box (or margin_box if no bounding box is being
      # used).  Text will flow onto the next page when it reaches the bottom of
      # the bounding box. Text wrap in Prawn does not re-flow linebreaks, so if
      # you want fully automated text wrapping, be sure to remove newlines
      # before attempting to draw your string.
      #
      # == Examples
      #
        ...
        if @indent_paragraphs
          string.split("\n").each do |paragraph|
            ...
            if remaining_text == paragraph
              # we were too close to the bottom of the page to print even one
              # line
